...
contract NFTMarket is Ownable {
    ...
    constructor(address erc20Addr, address erc721Addr) Ownable(msg.sender) {
    ...
    function list(uint256 _tokenId, uint256 _price) public {
        ...
        require(
            msg.sender == erc721Token.ownerOf(_tokenId),
            "not owner of nft"
        );
        require(
            erc721Token.isApprovedForAll(msg.sender, address(this)),
            "owner must ApprovedForAll to market first"
        );
    ...
    function buyNFT(uint256 _tokenId, uint256 _price) public {
        _buyNFT(_tokenId, _price, msg.sender);
    }

    // New function: permitBuy
    // Implements whitelist logic: only address with valid signature from owner can buy
    function permitBuy(
        uint256 _tokenId,
        uint256 _price,
    ...
    function _buyNFT(uint256 _tokenId, uint256 _price, address buyer) internal {
        require(priceOfNft[_tokenId] > 0, "NFT not in sale");
        require(
            _price >= priceOfNft[_tokenId],
            "buy price less than list price"
        );
        require(hookErc20.balanceOf(buyer) >= _price, "Insufficient funds");
        require(
        ...
        emit logBuy(buyer, _tokenId, _price);
    }
    ...
    function tokensReceived(
        ...
        if (data.length > 0) {
            ...
            require(
                (priceOfNft[tokenId] != 0) && (value >= priceOfNft[tokenId]),
                "NFT not in sale or not enough token received"
            );
            require(
                hookErc20.transfer(
