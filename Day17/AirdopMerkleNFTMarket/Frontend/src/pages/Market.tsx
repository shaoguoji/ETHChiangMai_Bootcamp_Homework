    ...
    useEffect(() => {
        ...
        const fetchListedTokens = async () => {
            try {
                ...
                // Fetch all NFTListed events to find any token that has ever been listed
                const logs = await publicClient.getLogs({
                    address: CONTRACTS.NFTMarket.address,
                    event: parseAbiItem('event NFTListed(address indexed seller, uint256 indexed tokenId, uint256 price)'),
                    fromBlock: fromBlock
                });
